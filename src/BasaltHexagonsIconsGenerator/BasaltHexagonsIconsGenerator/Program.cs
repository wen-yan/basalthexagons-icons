...
class Program
{
    public static void Main(string[] args)
    {
        ...
        {
            string path = $"../../../../../../icons/basalthexagons-{theme.Name}.svg";
            GenerateImages(new RenderContext(theme), $"../../../../../../icons", $"basalthexagons-{theme.Name}");
        }
    }
    ...
    private static void GenerateImages(RenderContext context, string directory, string baseFilename)
    {
        ...
        void SavePng(SvgDocument svgDoc, int size)
        {
            ...
            bitmap.Save($"{directory}/{baseFilename}-{size}x{size}.png", ImageFormat.Png);
        }

        SvgDocument svgDoc = GenerateSvg();
        svgDoc.Write($"{directory}/{baseFilename}.svg");
